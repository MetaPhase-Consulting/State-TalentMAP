...
describe('HomePageComponent', () => {
  ...
  it('can receive props', () => {
    const wrapper = shallow(<HomePagePositions {...props} />);
    expect(
      wrapper.find('HomePagePositionsSection').at(0).prop('positions').length,
    ).toBeGreaterThan(0);
  });
  ...
  it('displays proper title and icon when featuredGradeAndSkillPositions displayed', () => {
    ...
    expect(wrapper.find('HomePagePositionsSection').at(0).prop('title')).toBe(
      'Featured Positions That Match Your Grade And Skill(s)',
    );
    expect(wrapper.find('HomePagePositionsSection').at(0).prop('icon')).toBe(
      'bolt',
    );
  ...
  it('displays proper title and icon when recommendedGradeAndSkillPositions displayed', () => {
    ...
    expect(wrapper.find('HomePagePositionsSection').at(1).prop('title')).toBe(
      'Positions That Match Your Grade And Skill(s)',
    );
    expect(wrapper.find('HomePagePositionsSection').at(1).prop('icon')).toBe(
      'briefcase',
    );
    expect(wrapper.find('HomePagePositionsSection')).toHaveLength(2);
  });
  ...
  it('displays proper title and icon when featuredGradePositions displayed', () => {
    const wrapper = shallow(
      <HomePagePositions
        {...props}
        homePageFeaturedPositions={{
          ...props.homePageFeaturedPositions,
          name: 'featuredGradePositions',
        }}
      />,
    );
    expect(wrapper.find('HomePagePositionsSection').at(0).prop('title')).toBe(
      'Featured Positions That Match Your Grade',
    );
    expect(wrapper.find('HomePagePositionsSection').at(0).prop('icon')).toBe(
      'bolt',
    );
  ...
  it('displays proper title and icon when recommendedGradePositions displayed', () => {
    const wrapper = shallow(
      <HomePagePositions
        {...props}
        homePageRecommendedPositions={{
          ...props.homePageRecommendedPositions,
          name: 'recommendedGradePositions',
        }}
      />,
    );
    expect(wrapper.find('HomePagePositionsSection').at(1).prop('title')).toBe(
      'Positions That Match Your Grade',
    );
    expect(wrapper.find('HomePagePositionsSection').at(1).prop('icon')).toBe(
      'briefcase',
    );
