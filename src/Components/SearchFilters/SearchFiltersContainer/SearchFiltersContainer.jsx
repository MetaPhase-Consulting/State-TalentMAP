...
import {
  get, includes, indexOf, remove, sortBy,
} from 'lodash';
...
class SearchFiltersContainer extends Component {
  ...
  render() {
    ...
    const multiSelectFilterNamesTandemCommon = [
      'post',
      'postDiff',
      'dangerPay',
      'postIndicators',
    ...
    const multiSelectFilterNamesTandem1 = [
      'bidSeason',
      'bidCycle',
      'skill',
      'grade',
    ...
    const multiSelectFilterNamesTandem2 = [
      'bidSeason-tandem',
      'bidCycle-tandem',
      'skill-tandem',
      'grade-tandem',
    ...
    if (!useUS()) {
      ...
      remove(
        multiSelectFilterNamesTandemCommon,
        (f) => f === 'unaccompaniedStatus',
      );
    }
    ...
    const projectedVacancyFilter = sortedToggleNames.length
      ? get(toggleFiltersMap.get('projectedVacancy'), 'data')
      : null;
    const tandemFilter = sortedToggleNames.length
      ? get(toggleFiltersMap.get('tandem-toggle'), 'data')
      : null;
    const tandemIsSelected = tandemFilter
      ? tandemFilter.find((f) => f.code === 'tandem').isSelected
      : false;

    // post should come before TOD
    multiSelectFilterNames.splice(
      indexOf(multiSelectFilterNames, 'tod'),
      0,
      'post',
    );
    // END TOGGLE FILTERS
    ...
    const functionalBureaus = filters$
      .slice()
      .find((f) => f.item.description === 'functionalRegion');
    const functionalBureausTandem = filters$
      .slice()
      .find((f) => f.item.description === 'functionalRegion-tandem');

    // special handling for is_domestic filter
    const domesticFilter = (filters$ || []).find(
      (f) => f.item.description === 'domestic',
    );
    const overseasFilterData = propOrDefault(domesticFilter, 'data', []).find(
      (d) => d.code === 'false',
    );
    const domesticFilterData = propOrDefault(domesticFilter, 'data', []).find(
      (d) => d.code === 'true',
    );
    const overseasIsSelected = propOrDefault(
      overseasFilterData,
      'isSelected',
      false,
    );
    const domesticIsSelected = propOrDefault(
      domesticFilterData,
      'isSelected',
      false,
    );
    ...
    // commuter posts
    const commuterPosts = (filters$ || []).find(
      (f) => f.item.description === 'commuterPosts',
    );

    // get skill cones
    const skillCones = (filters$ || []).find(
      (f) => f.item.description === 'skillCone',
    );

    // get language groups
    const languageGroups = (filters$ || []).find(
      (f) => f.item.description === 'languageGroup',
    );
    ...
    [...multiSelectFilterNames, ...multiSelectFilterNamesTandem2].forEach(
      (n) => {
        ...
        if (n === 'post') {
          ...
          suggestions = mapDuplicates(
            postSearchResults.map((m) => ({
              ...m,
              location$: `${m.location.country}-${m.location.city}-${m.location.state}`,
            })),
            'location$',
          );
            ...
            default: {
              const getQueryProperty = () => {
                if (
                  ...
                  || type === 'bidCycle'
                  || type === 'bidSeason'
                  || type === 'bidCycle-tandem'
                  || type === 'bidSeason-tandem'
                ) {
                  return '_id';
                }
                return 'code';
        ...
        if (item && !includes(blackList, n)) {
          ...
          const isTandemCommon$ = includes(
            multiSelectFilterNamesTandemCommon,
            n,
          );
          const isPrimary = includes(multiSelectFilterNames, n);
          let accordionClass = isProjectedVacancy
            ? ' accordion-pv'
            : 'accordion-ap';
          if (isTandemSearch && !isTandemCommon$) {
            accordionClass = isTandem1
              ? ' accordion-tandem-1'
              : ' accordion-tandem-2';
          }
          const obj = {
    ...
    const commonContainerClass = tandemIsSelected
      ? 'tandem-common-filters'
      : '';
    const tandemUserClass = showTandem2
      ? 'tandem-2-filters'
      : 'tandem-1-filters';
    ...
    return (
      <div className={apContainerClass}>
        {tandemIsSelected && (
          <div className="tandem-filter-header tandem-filter-header--first">
            Tandem Filters
          </div>
        )}
          ...
          <div className={commonContainerClass}>
            <MultiSelectFilterContainer
              multiSelectFilterList={
                tandemIsSelected ? sortedFiltersTandemCommon : sortedFilters
              }
              queryParamToggle={this.props.queryParamToggle}
            />
          </div>
