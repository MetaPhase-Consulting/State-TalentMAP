...
class BureauFilter extends Component {
  ...
  onFunctionalBureauCheckBoxClick = (value, props) => {
    ...
    this.props.queryParamToggle(
      functionalBureaus.item.selectionRef,
      props[queryProperty],
      !value,
    );
  };
  ...
  render() {
    ...
    const functionalBureaus$ = functionalBureaus.data.filter(
      (b) => !b.is_regional,
    );
      ...
      <div className="usa-grid-full tm-nested-accordions">
        <Accordion>
          <AccordionItem
            ...
            id={`regional-bureau-sub-accordion${isTandem ? '-tandem' : ''}`}
            title="Regional Bureaus"
          ...
          <AccordionItem
            className="accordion-content-small"
            id={`functional-bureau-sub-accordion${isTandem ? '-tandem' : ''}`}
