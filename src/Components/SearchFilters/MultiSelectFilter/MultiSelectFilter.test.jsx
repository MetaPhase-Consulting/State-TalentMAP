...
describe('MultiSelectFilterComponent', () => {
  ...
  it('can receive props', () => {
    const wrapper = shallow(
      <MultiSelectFilter queryParamToggle={() => {}} item={item} />,
    );
  ...
  it('can call the onCheckBoxClick function', () => {
    const spy = sinon.spy();
    const wrapper = shallow(
      <MultiSelectFilter queryParamToggle={spy} item={item} />,
    );
    wrapper
      .instance()
      .onCheckBoxClick(1, { selectionRef: 'test', code: 'code' });
    sinon.assert.calledOnce(spy);
  });

  it('creates term titles when groupAlpha is true', () => {
    const wrapper = shallow(
      <MultiSelectFilter queryParamToggle={() => {}} item={item} groupAlpha />,
    );
