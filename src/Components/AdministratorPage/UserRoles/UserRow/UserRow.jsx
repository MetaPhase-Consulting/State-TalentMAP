...
class UserRow extends Component {
  checkPermission(permission) {
    ...
    const userPerms = this.props.permissionGroups.map(
      (permObj) => permObj.name,
    );
  ...
  render() {
    ...
    const tdArray = [];
    Object.keys(this.props.delegateRoles).forEach((role) => tdArray.push(
      <td
        key={get(delegateRoles, `${role}.group_name`)}
      ...
      >
        <CheckBox
          ...
          value={this.checkPermission([
            get(delegateRoles, `${role}.group_name`),
          ])}
...
export const mapDispatchToProps = (dispatch) => ({
  modifyPermission: (addRole, groupID, userID) => dispatch(modifyPermission(addRole, groupID, userID)),
});
