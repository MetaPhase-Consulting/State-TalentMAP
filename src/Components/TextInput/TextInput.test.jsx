...
describe('TextInputComponent', () => {
  ...
  it('calls changeText() on text change', () => {
    ...
    let wrapper;
    act(() => {
      wrapper = mount(<TextInput id={id} changeText={spy} />);
    });
  ...
  it('renders a label', () => {
    const text = 'test';
    const wrapper = shallow(<TextInput id={id} label={text} />);
    expect(wrapper.find(text)).toBeDefined();
  });

  it('renders a label message', () => {
    const text = 'test';
    const wrapper = shallow(<TextInput id={id} labelMessage={text} />);
    expect(wrapper.find(text)).toBeDefined();
  });

  it('renders a sr-only class', () => {
    const wrapper = shallow(<TextInput id={id} labelSrOnly />);
    expect(wrapper.find('.usa-sr-only')).toBeDefined();
  });

  it('renders different classNames based on type', () => {
    let wrapper = shallow(<TextInput id={id} type="success" />);
    expect(wrapper.find('.usa-input-success')).toBeDefined();
    wrapper = shallow(<TextInput id={id} type="error" />);
    ...
    wrapper = shallow(<TextInput id={id} type="focus" />);
