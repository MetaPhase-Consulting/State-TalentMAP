...
const PositionInformation = ({ assignment = {} }) => {
  ...
  const formattedAssignmentDates = assignmentStartDate && assignmentEndDate ? (
    <StartEnd start={assignmentStartDate} end={assignmentEndDate} />
  ) : (
    NO_ASSIGNMENT_DATE
  );
  const formattedBureau = propOrDefault(
    assignment,
    'position.bureau',
    NO_BUREAU,
  );
  const formattedPosition = propOrDefault(
    assignment,
    'position.title',
    NO_ASSIGNMENT_POSITION,
  );
  ...
  return (
    <div className="usa-grid-full">
      <div className="section-padded-inner-container">
        ...
        />
        <InformationDataPoint title="Bureau" content={formattedBureau} />
        <InformationDataPoint
          title="Position title"
          content={formattedPosition}
        />
        <InformationDataPoint title="Skill" content={formattedSkill} />
      </div>
    </div>
  );
};
