...
describe('BidTrackerCardListComponent', () => {
  ...
  it('is defined', () => {
    const wrapper = shallow(<BidTrackerCardList {...props} />);
  ...
  it('pushes the priority bid to the top', () => {
    ...
    const wrapper = shallow(<BidTrackerCardList {...props} bids={bidList} />);
    expect(
      wrapper.find('BidTrackerCardContainer').at(0).props('bid').bid.is_priority,
    ).toBe(true);
    expect(
      wrapper.find('BidTrackerCardContainer').at(1).props('bid').bid.is_priority,
    ).toBe(false);
  });
