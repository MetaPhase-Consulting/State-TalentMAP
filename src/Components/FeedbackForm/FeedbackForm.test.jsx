...
describe('FeedbackFormComponent', () => {
  ...
  it('displays "Sending" text when submission is sending', () => {
    const wrapper = shallow(<FeedbackForm {...props} feedbackIsSending />);
    expect(wrapper).toBeDefined();
    expect(wrapper.find('.feedback-submission-messages').text()).toBe(
      'Sending...',
    );
  });
  ...
  it('displays the success text when feedbackSuccess is true', () => {
    const wrapper = shallow(<FeedbackForm {...props} feedbackSuccess />);
    expect(wrapper).toBeDefined();
    expect(wrapper.find('.feedback-submission-messages').text()).toBe(
      'Submitted!',
    );
  ...
  it('can close the feedback', () => {
    const spy = sinon.spy();
    const wrapper = shallow(<FeedbackForm {...props} toggleVisibility={spy} />);
  ...
  it('matches snapshot when visible is false', () => {
    const wrapper = shallow(<FeedbackForm {...props} visible={false} />);
    expect(toJSON(wrapper)).toMatchSnapshot();
  });
});
