...
import {
  BID_PORTFOLIO_FILTERS,
  BID_PORTFOLIO_FILTERS_TYPE,
  BID_PORTFOLIO_SORTS,
  BID_PORTFOLIO_SORTS_TYPE,
  CLIENTS_PAGE_SIZES,
  UNASSIGNED_BIDDERS_FILTERS,
} from 'Constants/Sort';
import {
  filter, findIndex, get, includes, isEqual,
} from 'lodash';
...
class BidControls extends Component {
  ...
  UNSAFE_componentWillMount() {
    if (
      !(
        ...
        && get(this.props, 'selection[0].isCurrentUser', false)
      )
    ) {
    ...
    if (!useUnassignedFilter()) {
      BID_PORTFOLIO_FILTERS$.options = BID_PORTFOLIO_FILTERS.options.filter(
        (b) => b.value !== 'unassigned_filters',
      );
    }
    this.setState({
      filterBy:
        BID_PORTFOLIO_FILTERS$.options[
          findIndex(
            BID_PORTFOLIO_FILTERS$.options,
            (o) => o.value === this.props.defaultHandshake,
          )
        ],
    });
    if (
      this.props.defaultHandshake === 'unassigned_filters'
      && this.state.bidSeasons.length
    ) {
      this.setState({ unassignedFilter: true });
    }
    if (this.props.unassignedSelection.length) {
      this.setState(
        { unassignedBidders: this.props.unassignedSelection },
        this.generatePills,
      );
    }
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props.selection, nextProps.selection)) {
      if (
        !(
          nextProps.selection.length === 1
          && get(nextProps, 'selection[0].isCurrentUser', false)
        )
      ) {
  ...
  generatePills = () => {
    ...
    this.state.bidSeasons.forEach((a) => {
      pills.push({
        ...
        selectionRef: 'bidSeasons',
        codeRef: a.id,
      });
    });
    if (
      !includes(['', 'unassigned_filters'], get(filterBy, 'value', ''))
      && this.state.bidSeasons.length
    ) {
      pills.push({
    ...
    const renderUnassigned = this.state.bidSeasons.length
      && isEqual('unassigned_filters', get(filterBy, 'value'));
    if (renderUnassigned) {
      this.state.unassignedBidders.forEach((a) => {
        pills.push({
          description: a.text,
          selectionRef: 'unassignedBidders',
          codeRef: a.value,
        });
      });
    }
  ...
  pillClick = (dropdownID, pillID) => {
    switch (dropdownID) {
      case 'proxyCdos':
        this.setState(
          {
            proxyCdos: filter(this.state.proxyCdos, (o) => o.id !== pillID),
          },
          this.generatePills,
        );
        break;
      case 'bidSeasons':
        this.updateMultiSelect(
          filter(this.state.bidSeasons, (o) => o.id !== pillID),
        );
        break;
      case 'filterBy':
        this.onFilterChange(BID_PORTFOLIO_FILTERS.options[0].value);
      ...
      case 'unassignedBidders':
        this.setState(
          {
            unassignedBidders: filter(
              this.state.unassignedBidders,
              (o) => o.value !== pillID,
            ),
          },
          this.generatePills,
      ...
      <div className="usa-grid-full portfolio-controls">
        <div className="usa-width-one-whole portfolio-sort-container results-dropdown">
          ...
          >
            ...
            <CDOAutoSuggest cdoPills={proxyCdos} />
          </div>
          {displayCDOSeasonFilter && (
            <div className="portfolio-sort-container-contents small-screen-stack">
              ...
              {displayUnassignedFilter && (
                <div
                  className={`unassigned-bidder-picker-container usa-form ${
                    !unassignedFilter ? 'unassigned-disabled' : ''
                  }`}
                >
        ...
        <div className="usa-width-one-whole portfolio-filter-pills-container">
          ...
          <ResultsPillContainer items={pills} onPillClick={this.pillClick} />
        </div>
      </div>
    );
  }
