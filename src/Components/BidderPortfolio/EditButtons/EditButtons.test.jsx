...
describe('SearchResultsExportLink', () => {
  ...
  ].map((m) => it(`responds with the correct prop "${m[1].type}" when button index ${m[0]} is clicked`, () => {
    let val;
    function onChange(v) {
      val = v;
    }
    const wrapper = shallow(
      <EditButtons onChange={onChange} showSave={m[2]} />,
    );
    if (m[2]) {
      wrapper.instance().changeSaveState(true);
    }
