...
import BidderPortfolioGridList, {
  isAllFalse,
  isAllTrue,
} from './BidderPortfolioGridList';
...
describe('BidderPortfolioGridListComponent', () => {
  ...
  it('is defined', () => {
    const wrapper = shallow(<BidderPortfolioGridList {...props} />);
  ...
  [
    ['componentDidMount'],
    ['onSetAccordion'],
    ['toggleExpand', { expandAll: false }],
    ['toggleExpand', { expandAll: true }],
  ].map((m) => it(`calls debouncedSetValue (setExpandAllValue child function) after ${m[0]} and new state === ${m[1]}`, (done) => {
    const wrapper = shallow(<BidderPortfolioGridList {...props} />);
