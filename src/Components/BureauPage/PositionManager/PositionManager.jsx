...
import {
  BUREAU_POSITION_SORT,
  POSITION_MANAGER_PAGE_SIZES,
} from 'Constants/Sort';
import {
  BUREAU_PERMISSIONS,
...
import {
  get, has, isEmpty, pick, sortBy, throttle, uniqBy,
} from 'lodash';
import {
  bureauPositionsFetchData,
  downloadBureauPositionsData,
  saveBureauUserSelections,
} from 'actions/bureauPositions';
...
const PositionManager = (props) => {
  ...
  const bureauPermissions$ = sortBy(
    bureauFilters.filters
      .find((f) => f.item.description === 'region')
      .data.map((a) => pick(a, ['code', 'short_description', 'long_description'])),
  ...
  const [ordering, setOrdering] = useState(
    userSelections.ordering || BUREAU_POSITION_SORT.options[0].value,
  );
  const [selectedGrades, setSelectedGrades] = useState(
    userSelections.selectedGrades || [],
  );
  const [selectedSkills, setSelectedSkills] = useState(
    userSelections.selectedSkills || [],
  );
  const [selectedPosts, setSelectedPosts] = useState(
    userSelections.selectedPosts || [],
  );
  const [selectedTODs, setSelectedTODs] = useState(
    userSelections.selectedTODs || [],
  );
  const [selectedCycles, setSelectedCycles] = useState(
    userSelections.selectedCycles || [],
  );
  const [selectedLanguages, setSelectedLanguages] = useState(
    userSelections.selectedLanguages || [],
  );
  const [selectedPostIndicators, setSelectedPostIndicators] = useState(
    userSelections.selectedPostIndicators || [],
  );
  const [selectedBureaus, setSelectedBureaus] = useState(
    userSelections.selectedBureaus || isAO
      ? [bureauPermissions$[0]]
      : [props.bureauPermissions[0]],
  );
  const [selectedOrgs, setSelectedOrgs] = useState(
    userSelections.selectedOrgs || [props.orgPermissions[0]],
  );
  ...
  const skillOptions = uniqBy(
    sortBy(skills.data, [(s) => s.description]),
    'code',
  );
  ...
  const organizationOptions = sortBy(organizations, [
    (o) => o.long_description,
  ]);
  ...
  const cycleOptions = uniqBy(
    sortBy(cycles.data, [(c) => c.custom_description]),
    'custom_description',
  );
  const languages = bureauFilters$.find(
    (f) => f.item.description === 'language',
  );
  const languageOptions = uniqBy(
    sortBy(languages.data, [(c) => c.custom_description]),
    'custom_description',
  );
  const postIndicators = bureauFilters$.find(
    (f) => f.item.description === 'postIndicators',
  );
  const postIndicatorsOptions = sortBy(postIndicators.data, [
    (c) => c.description,
  ]);
  ...
  const query = {
    ...
    [postIndicators.item.selectionRef]: selectedPostIndicators.map(
      (postIndObject) => get(postIndObject, 'code'),
    ),
    ordering,
    page,
    limit,
  ...
  const throttledTextInput = () => throttle((q) => setTextInput(q), 300, { leading: false, trailing: true });
  ...
  const getOverlay = () => {
    if (bureauPositionsIsLoading) {
      return (
        <Spinner
          type="bureau-results"
          class="homepage-position-results"
          size="big"
        />
      );
    }
    if (noBureausSelected && noOrgsSelected) {
      return (
        <Alert
          type="error"
          title="No bureau/organization selected"
          messages={[
            { body: 'Please select at least one bureau/organization filter.' },
          ]}
        />
      );
    }
    if (bureauPositionsHasErrored) {
      return (
        <Alert
          type="error"
    ...
    if (noResults) {
      return (
        <Alert
          ...
          messages={[
            { body: 'Please broaden your search criteria and try again.' },
          ]}
        />
      );
    }
  ...
  const resetFilters = () => {
    ...
    setSelectedBureaus(
      isAO
        ? [bureauPermissions$[0]].filter((f) => f)
        : [props.bureauPermissions[0]].filter((f) => f),
    );
    setSelectedCycles([]);
  ...
  useEffect(() => {
    ...
    const defaultBureauCode = isAO
      ? get(bureauPermissions$, '[0].code')
          ...
          <div className="results-search-bar padded-main-content results-single-search homepage-offset">
            <div className="usa-grid-full results-search-bar-container">
              ...
              <div className="usa-width-one-whole position-manager-filters results-dropdown">
                ...
                <PermissionsWrapper
                  permissions={['bureau_user', 'ao_user']}
                  minimum
                >
                  <div className="filter-div">
