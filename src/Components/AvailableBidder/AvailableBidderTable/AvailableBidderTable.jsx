...
import {
  availableBidderExport,
  availableBiddersFetchData,
...
const AvailableBidderTable = (props) => {
  ...
  // App state
  const biddersData = useSelector(
    (state) => state.availableBiddersFetchDataSuccess,
  );
  const availableBiddersIsLoading = useSelector(
    (state) => state.availableBiddersFetchDataLoading,
  );
  ...
  const bureaus = filterData.filters.find(
    (f) => f.item.description === 'region',
  );

  const bidders = isLoading
    ? [...new Array(10)]
    : get(biddersData, 'results', []);
  ...
  const tableHeaders = isCDO
    ...
    : ['Name', 'Skill', 'Grade', 'Languages', 'TED', 'Post', 'CDO'];
  ...
  const handleSort = (header) => header === sort ? setSort(`-${header}`) : setSort(header);
  ...
  const getTitleCount = () => {
    ...
    if (!isLoading) {
      if (isCDO) {
        bidderCountTitle = cdoView
          ? `(${bidders.length})`
          : `(${
            bidders.filter((b) => get(b, 'available_bidder_details.is_shared')).length
          })`;
      } else {
  ...
  return !bidders.length && !isLoading ? (
    ...
    <div className="usa-width-one-whole bidder-manager-bidders ab-lower-section">
      ...
      <table className="bidder-manager-bidders">
        <thead>
          <tr>
            {tableHeaders.map((item) => item !== 'Languages' && item !== 'Comments' ? (
                  ...
                  <ToggleButton
                    labelTextLeft={(
                      ...
                      >
                        <FA
                          name="street-view"
                          className={`fa-lg ${cdoView ? 'active' : ''}`}
                        />
                      </Tooltip>
                    )}
                    labelTextRight={(
