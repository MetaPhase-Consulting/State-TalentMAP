...
class SavedSearchesList extends Component {
  ...
  render() {
    ...
    const {
      savedSearches, goToSavedSearch, deleteSearch, mappedParams,
    } = this.props;

    savedSearches.results.slice().forEach((savedSearch) => savedSearchArray.push(
      <SavedSearchesListResultsCard
        ...
        isProjectedVacancy={
          savedSearch.endpoint === '/api/v1/fsbid/projected_vacancies/'
            || savedSearch.endpoint === '/api/v1/fsbid/projected_vacancies/tandem/'
        }
        isTandemSearch={
          savedSearch.endpoint
      ...
      <div className="usa-grid-full" ref={this.setContainerRef}>
        {!!savedSearchArray.length && (
          <ScrollArea {...this.state.scroll} ref={this.setScrollRef}>
            <div
              className={`usa-grid-full saved-searches-list ${emptyListClass}`}
            >
